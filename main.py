...
if brewingDay:
    ...
    while brewingDay:
        ...
        if stateNow == -1 and max(temp1_float, temp2_float) >= temp_all[stateNext]:
            ...
            df.to_csv(dateString + '/' + dateString.split('-')[0] + '_' + timeString + '_aufheizen.csv')
        ...
        if stateNow >= 0 and timeRast_Now >= time_all[stateNow]:
            ...
            if len(df.index) > 1:
                timeString = F.datetimeNowString().split('-')[1]
                df.to_csv(dateString + '/' + dateString.split('-')[0] + '_' + timeString + '_rast' + str(stateNow) + '.csv')
            ...
            if abmaischen:
                ...
                F.logger.info('Fertig abgemaischt --> brewingDay == False.')
                F.logger.info('Chart erstellen für folgenden Ordner: {}'.format(dateString))
                brewingDay = False
